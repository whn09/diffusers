...
class SeaCacheRootHook(ModelHook):
    ...
    def pre_forward(self, module: torch.nn.Module, *args, **kwargs):
        ...
        if self.state_manager._current_context is None:
            ...
            return args, kwargs
        # Scheduler coordinates come from the cache-context info the denoising loop attaches:
        # `transformer.cache_context(name, step=..., sigma=..., num_steps=...)`.
        context_info = HookRegistry.check_if_exists_or_initialize(module).context_info
        step_index = context_info.get("step")
        sigma = context_info.get("sigma")
        num_inference_steps = context_info.get("num_steps")
        if step_index is None or sigma is None or num_inference_steps is None:
            self.shared_state.mark_fail_open(
                ...
                "required: pass them as `cache_context(name, step=..., sigma=..., num_steps=...)`."
            )
            return args, kwargs
