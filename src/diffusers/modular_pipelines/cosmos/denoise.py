    ...
    def __call__(self, components: Cosmos3OmniModularPipeline, block_state: BlockState, i: int, t: torch.Tensor):
        ...
        for pass_name in prediction_passes:
            ...
            with components.transformer.cache_context(
                ...
                num_steps=components.scheduler.num_inference_steps,
            ):
...
class Cosmos3TransferLoopDenoiser(ModularPipelineBlocks):
    ...
    @staticmethod
    def _forward(components, static, vision_tokens, vision_timesteps, context_name, step, sigma, num_steps):
        with components.transformer.cache_context(context_name, step=step, sigma=sigma, num_steps=num_steps):
